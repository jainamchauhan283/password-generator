...
function select(value, obj) {
  if (!value) {
    ...
    obj.style.textDecoration = "none"; // Remove line-through if background is white
  } else {
    obj.style.backgroundColor = "#2e4057";
    obj.style.color = "white";
    obj.style.textDecoration = "line-through"; // Add line-through if background is black
  }
}
...
numbersButton.addEventListener("click", function () {
  ...
  if (includeNumbers) charStrings.push(value);
  else charStrings.splice(charStrings.indexOf(value), 1);
  getPassword();
  result.value = word;
  check();
});
...
lowerCaseButton.addEventListener("click", function () {
  ...
  if (includeLowerCase) charStrings.push(value);
  else charStrings.splice(charStrings.indexOf(value), 1);
  getPassword();
  result.value = word;
  check();
...
upperCaseButton.addEventListener("click", function () {
  ...
  if (includeUpperCase) charStrings.push(value);
  else charStrings.splice(charStrings.indexOf(value), 1);
  getPassword();
  result.value = word;
  check();
...
symbolsButton.addEventListener("click", function () {
  ...
  if (includeSymbols) charStrings.push(value);
  else charStrings.splice(charStrings.indexOf(value), 1);
  getPassword();
  result.value = word;
  check();
...
function getPassword() {
  if (parseInt(val.innerHTML) > 0) passwordLength = parseInt(val.innerHTML);
  word = "";
...
up.addEventListener("click", function f() {
  if (passwordLength < 20) passwordLength++;
...
function showMessage(message) {
  ...
  toast.style.opacity = "1"; // Ensure opacity is set to fully visible
  toast.style.bottom = "10px"; // Adjust top position to 10px from top
  // toast.style.r = "10px"; // Adjust right position to 10px from right
  setTimeout(function () {
